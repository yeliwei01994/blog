...
export function DiaryArticlePage() {
  ...
  useEffect(() => { const article = articleRef.current; if (!article) return; for (const pre of article.querySelectorAll('pre')) { if (pre.parentElement?.classList.contains('code-window')) continue; const code = pre.querySelector('code'); const rawCode = code?.textContent ?? pre.textContent ?? ''; const windowElement = document.createElement('div'); windowElement.className = 'code-window'; const title = document.createElement('div'); title.className = 'code-window__title'; const language = Array.from(code?.classList ?? []).find(item => item.startsWith('language-'))?.replace('language-', '').toUpperCase() ?? 'PLAINTEXT'; title.textContent = language; pre.parentNode?.insertBefore(windowElement, pre); windowElement.append(title, pre); } }, [entry.id]);
